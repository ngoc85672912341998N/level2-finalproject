...
export const getTodo = createAsyncThunk('todos/getTodos', async () => {
    return fetch("https://students.trungthanhweb.com/api/todo?apitoken=" + localStorage.getItem('api_token'))
        .then((res) => res.json());
})
const initialState = {
    task: [{id:1,note:"a",status:false}],
}
export const taskSlice = createSlice({
    ...
    reducers: {
        addTask: (state, action) => {
            ...
            data.append('apitoken', localStorage.getItem('api_token'));
